				...
				if (response.statusCode === 200) {
					...
					result.forEach((element) => {
						...
						if (eqInfo[1]) {
							...
							const latitude = Number.parseFloat(eqInfo[2]);
							const longitude = Number.parseFloat(eqInfo[3]);
							const depth = Number.parseFloat(eqInfo[4]);
							const magnitude = Number.parseFloat(eqInfo[6]);
