...
import type { Earthquakes } from "@prisma/client";
...
export async function GET(request: Request) {
	try {
		...
		const page = Number.parseInt(searchParams.get("page") || "1", 10); // Default to page 1 if not provided
		const limit = Number.parseInt(searchParams.get("limit") || "100", 10); // Default to 100 items per page if not provided
